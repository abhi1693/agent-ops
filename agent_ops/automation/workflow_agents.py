...
def is_agent_language_model_node_type(node_type: Any) -> bool:
    ...
    catalog_node = get_catalog_node(normalized_type)
    return catalog_node is not None and CAPABILITY_AGENT_MODEL in catalog_node.capabilities
...
def describe_agent_auxiliary_supported_sources(target_port: str) -> str:
    if target_port == AGENT_LANGUAGE_MODEL_INPUT_PORT:
        ...
        registry = initialize_workflow_catalog()
        model_node_types = {
            node.id
            for node in registry["node_types"].values()
            if CAPABILITY_AGENT_MODEL in node.capabilities
        }
        return ", ".join(sorted(model_node_types))
    if target_port == AGENT_TOOL_INPUT_PORT:
...
def normalize_workflow_agent_config(
    ...
    if isinstance(raw_input_modes, dict):
        normalized[WORKFLOW_INPUT_MODES_CONFIG_KEY] = dict(raw_input_modes)
